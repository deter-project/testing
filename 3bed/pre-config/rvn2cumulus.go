...
func main() {
	...
	for _, l := range topo.Links {
		...
		} else if n.Name == "steam" {
			configs[z.Name] = append(configs[z.Name], steamConfig(z.Port))
	    } else if n.Name == "leaf" {
...
//steam gets attached to the users network in access mode
func steamConfig(port string) Ifx {
	return Ifx{
		For:  "steam",
		Name: port,
		BridgeDefs: []string{
